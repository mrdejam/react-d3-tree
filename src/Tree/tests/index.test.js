...
describe('<Tree />', () => {
  ...
  it('maps every parent-child relation onto a <Link /> with expected siblings', () => {
    const linkCount = 5; // 1 top level node + 2 child nodes (1 child, 2 subs) in mockData
  ...
  it("reassigns internal props if `props.data`'s array reference changes", () => {
    ...
    nextData[0].subs.push({ name: `${nextData[0].subs.length}` });
    const renderedComponent = mount(<Tree data={mockData} />);
    expect(Tree.assignInternalProperties).toHaveBeenCalledTimes(mockDataDepth);
  ...
  describe('orientation', () => {
    it('passes `props.orientation` to its <Node /> and <Link /> subs', () => {
  ...
  describe('collapsible', () => {
    it("collapses a node's subs when it is clicked in an expanded state", () => {
    ...
    it("expands a node's subs when it is clicked in a collapsed state", () => {
      const renderedComponent = mount(<Tree data={mockData} />);
      const nodeCount = renderedComponent.find(Node).length;
      renderedComponent
        .find(Node)
