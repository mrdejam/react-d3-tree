...
class Tree extends React.Component<TreeProps, TreeState> {
  ...
  static assignInternalProperties(data: RawNodeDatum[], currentDepth: number = 0): TreeNodeDatum[] {
    ...
    return d.map(n => {
      ...
      // If there are subs, recursively assign properties to them too.
      if (nodeDatum.subs && nodeDatum.subs.length > 0) {
        nodeDatum.subs = Tree.assignInternalProperties(nodeDatum.subs, currentDepth + 1);
      }
      return nodeDatum;
    });
  }
  ...
  findNodesById(nodeId: string, nodeSet: TreeNodeDatum[], hits: TreeNodeDatum[]) {
    ...
    nodeSet.forEach(node => {
      if (node.subs && node.subs.length > 0) {
        hits = this.findNodesById(nodeId, node.subs, hits);
      }
    });
    return hits;
  }
  ...
  findNodesAtDepth(depth: number, nodeSet: TreeNodeDatum[], accumulator: TreeNodeDatum[]) {
    ...
    nodeSet.forEach(node => {
      if (node.subs && node.subs.length > 0) {
        accumulator = this.findNodesAtDepth(depth, node.subs, accumulator);
      }
    });
    return accumulator;
  }
   ...
   * the passed `TreeNodeDatum` and its subs to `true`.
   *
   * @static
   */
  ...
  generateTree() {
    ...
    const rootNode = tree(
      hierarchy(this.state.data[0], d => (d.__rd3t.collapsed ? null : d.subs))
    );
  ...
  getNodeClassName = (parent: HierarchyPointNode<TreeNodeDatum>, nodeDatum: TreeNodeDatum) => {
    ...
    if (hasParent) {
      return nodeDatum.subs ? branchNodeClassName : leafNodeClassName;
    } else {
