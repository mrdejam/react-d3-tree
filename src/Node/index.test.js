...
describe('<Node />', () => {
  ...
  it('applies correct base className if `data.subs` is defined and not empty', () => {
    const leafNodeComponent = shallow(<Node {...mockProps} />);
    const leafNodeComponentWithEmptysubs = shallow(
      <Node {...mockProps} data={{ ...data, subs: [] }} />
    );
    const nodeComponent = shallow(
      <Node {...mockProps} data={{ ...data, subs: [{ name: 'leaf' }] }} />
    );
    ...
    expect(
      leafNodeComponentWithEmptysubs
        .find('g')
        .first()
        .prop('className')
    ).toBe('rd3t-leaf-node');
