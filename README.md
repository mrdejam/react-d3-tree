<h1 align="center">MRDEJAM - React D3 Tree</h1>
...
// Note how deeper levels are defined recursively via the `subs` property.
const orgChart = {
  name: 'CEO',
  subs: [
    {
...
- Nodes can have further `RawNodeDatum` objects nested inside them via the `subs` key, creating a hierarchy from which the tree graph can be generated.
...
- `branchNodeClassName` - applied to any node with 1+ subs.
- `leafNodeClassName` - applied to any node without subs.
...
function StyledLinksTree() {
  const getDynamicPathClass = ({ source, target }, orientation) => {
    if (!target.subs) {
      // Target node has no subs -> this link leads to a leaf node.
